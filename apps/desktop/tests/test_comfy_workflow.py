...
class ComfyWorkflowTests(unittest.TestCase):
    ...
    def test_first_last_graph_binds_individual_immutable_assets(self) -> None:
        ...
        self.assertEqual(plan.workflow["14"]["class_type"], "ImageScale")
        self.assertEqual(plan.workflow["14"]["inputs"]["image"], ["9", 0])
        self.assertEqual(plan.workflow["14"]["inputs"]["width"], 832)
        self.assertEqual(plan.workflow["14"]["inputs"]["height"], 480)
        self.assertEqual(plan.workflow["15"]["class_type"], "ImageScale")
        self.assertEqual(plan.workflow["15"]["inputs"]["image"], ["10", 0])
        self.assertEqual(plan.workflow["5"]["inputs"]["start_image"], ["14", 0])
        self.assertEqual(plan.workflow["5"]["inputs"]["end_image"], ["15", 0])
        self.assertEqual(plan.workflow["11"]["inputs"]["clip_name"], "clip_vision_h.safetensors")
        self.assertEqual(plan.workflow["12"]["inputs"]["image_1"], ["14", 0])
        self.assertEqual(plan.workflow["12"]["inputs"]["image_2"], ["15", 0])
